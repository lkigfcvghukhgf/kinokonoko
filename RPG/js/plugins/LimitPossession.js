...
Imported['LimitPossession'] = 1.01;

/*:
 * @plugindesc ver1.01/アイテムの所持を重量で制限する仕組みを追加します。
 * @author Yana
 *
 * @param WeightText
 * @desc 重量の表示名です。
...
(function() {
    ...
    Window_ItemList.prototype.processTouch = function() {
        if (this._aButtonWindow && this.isOpenAndActive() &&
            TouchInput.isTriggered() && this.isTouchedInsideAbolitionFrame()) {
    ...
    Scene_Item.prototype.updateReserveItems = function() {
        ...
        if (ary.length > 0 && this._itemWindow) this._itemWindow.refresh();
    };
